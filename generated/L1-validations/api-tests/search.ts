...
export default function search(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function searchTest(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                testObj._EXTERNAL = input.externalData;
                const item = payloadUtils.getJsonPath(
                    testObj,
                    "$.context.action",
                );
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition validate_attribute_4**: all of the following sub conditions must be met:
