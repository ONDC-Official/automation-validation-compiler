...
export default function on_init(input: validationInput): validationOutput {
  ...
  - **condition REQUIRED_CONTEXT_ACTION_9.2**: every element of $.context.action must be in ["on_init"]`,
        ...
        function on_init_Message_TESTS(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
  ...
  - **condition REQUIRED_MESSAGE_TYPE_23.2**: every element of $.message.order.fulfillments[*].type must be in ["DELIVERY", "SELF_PICKUP"]`,
                ...
                function REQUIRED_MESSAGE_TYPE_24(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_TYPE_24**: $.message.order.fulfillments[*].type must be present in the payload`,
                                },
                            ];
                        }
                ...
                function REQUIRED_MESSAGE_GPS_25(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_GPS_25**: $.message.order.fulfillments[*].stops[*].location.gps must be present in the payload`,
                                },
                            ];
                        }
                ...
                function REQUIRED_MESSAGE_CATEGORY_26(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_CATEGORY_26**: all of the following sub conditions must be met:
  ...
  - **condition REQUIRED_MESSAGE_CATEGORY_26.2**: every element of $.message.order.fulfillments[*].vehicle.category must be in ["AUTO_RICKSHAW", "CAB", "TWO_WHEELER"]`,
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_VALUE_29**: $.message.order.quote.price.value must be present in the payload`,
                                },
                            ];
                        }
                ...
                function REQUIRED_MESSAGE_CURRENCY_30(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_CURRENCY_30**: $.message.order.quote.price.currency must be present in the payload`,
                                },
                            ];
                        }
                ...
                function REQUIRED_MESSAGE_TITLE_31(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_TITLE_31**: all of the following sub conditions must be met:

  - **condition REQUIRED_MESSAGE_TITLE_31.1**: $.message.order.quote.breakup[*].title must be present in the payload
  - **condition REQUIRED_MESSAGE_TITLE_31.2**: every element of $.message.order.quote.breakup[*].title must be in ["BASE_FARE", "DISTANCE_FARE", "CANCELLATION_CHARGES", "REFUND", "TOLL_CHARGES", "PARKING_CHARGES", "TAX", "WAITING_CHARGES", "DRIVER_BATA", "NIGHT_CHARGES", "ADD_ONS"]`,
                                },
                            ];
                        }
                ...
                function REQUIRED_MESSAGE_CURRENCY_32(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_CURRENCY_32**: $.message.order.quote.breakup[*].price.currency must be present in the payload`,
                                },
                            ];
                        }
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_VALUE_33**: $.message.order.quote.breakup[*].price.value must be present in the payload`,
                                },
                            ];
                        }
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_ID_34**: $.message.order.payments[*].id must be present in the payload`,
                                },
                            ];
                        }
                ...
                function REQUIRED_MESSAGE_COLLECTED_BY_35(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_COLLECTED_BY_35**: all of the following sub conditions must be met:
  ...
  - **condition REQUIRED_MESSAGE_COLLECTED_BY_35.2**: every element of $.message.order.payments[*].collected_by must be in ["BPP", "BAP"]`,
                ...
                function REQUIRED_MESSAGE_STATUS_36(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_STATUS_36**: all of the following sub conditions must be met:
  ...
  - **condition REQUIRED_MESSAGE_STATUS_36.2**: every element of $.message.order.payments[*].status must be in ["NOT-PAID", "PAID"]`,
                ...
                function REQUIRED_MESSAGE_TYPE_37(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_TYPE_37**: all of the following sub conditions must be met:
  ...
  - **condition REQUIRED_MESSAGE_CODE_41.2**: every element of $.message.order.cancellation_terms[*].fulfillment_state.descriptor.code must be in ["RIDE_CANCELLED", "RIDE_ENDED", "RIDE_STARTED", "RIDE_ASSIGNED", "RIDE_ENROUTE_PICKUP", "RIDE_ARRIVED_PICKUP", "RIDE_CONFIRMED"]`,
                ...
                function REQUIRED_MESSAGE_REASON_REQUIRED_42(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition REQUIRED_MESSAGE_REASON_REQUIRED_42**: $.message.order.cancellation_terms[*].reason_required must be present in the payload`,
                                },
                            ];
                        }
	...
	> - **condition B**: $.message.order.fulfillments[*].stops[*].authorization.status must **not** be present in the payload`,
                ...
                function VALID_ENUM_MESSAGE_CODE_5(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
                            ...
                            return [
                                {
                                    valid: false,
                                    code: 30000,
                                    description: `- **condition validate_tag_0**: every element of $.message.order.tags[*].descriptor.code must be in ["BPP_TERMS"]
	...
	> - **condition B**: $.message.order.fulfillments[*].tags[*].descriptor.code must **not** be present in the payload`,
                ...
                const testFunctions: testFunctionArray = [
                    ...
                    REQUIRED_MESSAGE_TYPE_24,
                    REQUIRED_MESSAGE_GPS_25,
                    REQUIRED_MESSAGE_CATEGORY_26,
                    REQUIRED_MESSAGE_NAME_27,
                    REQUIRED_MESSAGE_PHONE_28,
                    REQUIRED_MESSAGE_VALUE_29,
                    REQUIRED_MESSAGE_CURRENCY_30,
                    REQUIRED_MESSAGE_TITLE_31,
                    REQUIRED_MESSAGE_CURRENCY_32,
                    REQUIRED_MESSAGE_VALUE_33,
                    REQUIRED_MESSAGE_ID_34,
                    REQUIRED_MESSAGE_COLLECTED_BY_35,
                    REQUIRED_MESSAGE_STATUS_36,
                    REQUIRED_MESSAGE_TYPE_37,
                    REQUIRED_MESSAGE_CODE_41,
                    REQUIRED_MESSAGE_REASON_REQUIRED_42,
