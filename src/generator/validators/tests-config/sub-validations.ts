...
export class ErrorCodeValidator extends TestObjectValidator {
    ...
    validate = async () => {
        ...
        if (
            typeof this.targetObject[TestObjectSyntax.SuccessCode] !== "number"
        ) {
            throw new Error(
...
export class VariableValidator extends TestObjectValidator {
    ...
    validate = async () => {
        for (const key in this.targetObject) {
            if (
                Object.values(TestObjectSyntax).includes(
                    key as TestObjectSyntax,
                )
            ) {
                continue;
            }
